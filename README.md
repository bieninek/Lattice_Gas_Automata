...
public class FileUtil {
    public void saveToFile(BufferedImage img) throws IOException {
        ImageIO.write(img, "bmp",
                new File("C:\\Users\\bbart\\Desktop\\studia\\V semestr materiały\\modelowanie" +
                        "\\cwiczenia\\6_gaz_siatkowy\\input.png"));
    }

    public BufferedImage openFromFile() {
        try {
            return ImageIO.read(new File("C:\\Users\\bbart\\Desktop\\studia\\V semestr materiały\\modelowanie" +
                    "\\cwiczenia\\6_gaz_siatkowy\\input.png"));
...
```java
if (cells.get(i).get(j).getType().equals(Cell.cellType.BORDER)) {
    Cell newCell = new Cell(Cell.cellType.BORDER);
    ArrayList<Cell> line = cells.get(i);
    line.set(j, newCell);
    newCells.set(i, line);
```
...
```java
if (state.get(0).equals(1) && cellUp.getType().equals(Cell.cellType.BORDER)) {
    newState.set(0, 0);
    newState.set(2, 1); 

} else if (state.get(1).equals(1) && cellRight.getType().equals(Cell.cellType.BORDER)) {
    newState.set(1, 0);
    newState.set(3, 1); 

} else if (state.get(2).equals(1) && cellDown.getType().equals(Cell.cellType.BORDER)) {
    newState.set(2, 0); 
    newState.set(0, 1);

} else if (state.get(3).equals(1) && cellLeft.getType().equals(Cell.cellType.BORDER)) {
    newState.set(3, 0);
    newState.set(1, 1);
```
...
if (cellUp.getState().get(2).equals(1)) {
    ...
    Cell tmp = new Cell(Cell.cellType.NORMAL);
    ArrayList<Integer> tmpArr = cellUp.getState();
    tmpArr.set(2, 0);
    tmp.setState(tmpArr);
    ArrayList<Cell> line = cells.get(i-1);
    line.set(j, tmp);
    newCells.set(i, line); 
}
